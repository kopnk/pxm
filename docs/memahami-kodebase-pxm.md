...
|---|---|---|
| Frontend UI | `frontend/pages/`, `frontend/components/` | Menampilkan list, form, dashboard, filter, dan chart |
| Business logic client | `frontend/composables/` | Tempat API call, mapping data, helper form/list |
| State | `frontend/stores/` | Menyimpan state saja |
| API server | `frontend/server/api/` | Validasi, otorisasi, operasi domain, response standar |
| Middleware server | `frontend/server/middleware/` | Memastikan request `/api/*` punya sesi valid |
| Data access | `frontend/server/utils/*Store.ts` | Adapter DynamoDB single-table per domain |
| Validasi input | `frontend/server/validation/*.schema.ts` | Semua body/query endpoint pakai Zod |
| Infra | `infrastructure/` | Package AWS CDK untuk stage `dev` dan `prod` |
...
| Path | Fungsi |
|---|---|
| `frontend/nuxt.config.ts` | Konfigurasi global Nuxt |
| `frontend/app.vue` | Root app |
| `frontend/assets/scss/main.scss` | SCSS utama + utility class visual |
| `frontend/utils/apiFetch.ts` | Wrapper request |
| `infrastructure/` | AWS CDK stack dan konfigurasi stage |
...
| `frontend/components/` | Komponen presentational reusable |
| `frontend/composables/` | Logic API/form/list/dashboard |
| `frontend/stores/` | State Pinia per domain |
| `frontend/lib/` | Helper murni + konstanta shared lintas client/server |
| `frontend/utils/` | Helper frontend ringan seperti fetch wrapper dan formatter UI/export |
| `frontend/middleware/` | Guard route frontend |

### Backend

| Path | Fungsi |
|---|---|
| `frontend/server/api/` | Endpoint REST internal Nuxt |
| `frontend/server/utils/*Store.ts` | CRUD/query DynamoDB per domain |
| `frontend/server/utils/` | Helper runtime backend: auth, response, audit, storage, PDF, query helpers |
| `frontend/server/utils/cognitoAuth.ts` | Integrasi auth ke Cognito |
| `frontend/server/utils/audit.ts` | Audit log ke DynamoDB |
| `frontend/server/validation/` | Zod schema input |
...
3. `frontend/scripts/dev/dev-stage-dev.mjs`
4. `frontend/server/api/auth/login.post.ts`
5. `frontend/server/utils/cognitoAuth.ts`
6. Salah satu alur CRUD lengkap, misalnya:
   - `frontend/pages/clients/index.vue`
   - `frontend/composables/useClientsApi.ts`
   - `frontend/stores/clients.ts`
   - `frontend/server/api/clients/index.get.ts`
   - `frontend/server/utils/clientStore.ts`
7. `frontend/server/utils/projectStore.ts` dan domain store terkait

## 9) Checklist Saat Menambah Fitur

1. Tentukan access pattern DynamoDB-nya.
2. Tambah/ubah validation di `frontend/server/validation/`.
3. Tambah/ubah store helper di `frontend/server/utils/`.
4. Tambah/ubah endpoint `frontend/server/api/...`.
5. Tambah/ubah composable di `frontend/composables/useXxxApi.ts`.
