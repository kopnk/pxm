...
export function matchesProjectFinancialTaxSectionKind(
  kind: "taxIn" | "taxOut" | "pph",
  item: Pick<ProjectFinancialRecord, "flowDirection" | "taxIn" | "taxOut" | "pph">,
) {
  if (kind === "taxIn") {
    return item.flowDirection === "in" && Number(item.taxIn ?? 0) > 0;
  }

  if (kind === "taxOut") {
    return item.flowDirection === "out" && Number(item.taxOut ?? 0) > 0;
  }

  return item.flowDirection === "in" && Number(item.pph ?? 0) > 0;
}
