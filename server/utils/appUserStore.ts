...
export async function ensureAppUserPermissionsForRole(
  userId: string,
  role: string,
) {
  const current = await getAppUserRecordById(userId);
  ...
  return setAppUserPermissions(userId, defaultMatrixForRole(role));
}
...
export async function syncLegacyUserRecordById(userId: string) {
  return getAppUserRecordById(userId);
}
