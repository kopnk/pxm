...
npx cdk deploy -c stage=dev --outputs-file .cdk-outputs/dev.json

npx cdk synth -c stage=prod
npx cdk deploy -c stage=prod --outputs-file .cdk-outputs/prod.json
```
...
```bash
npx cdk deploy -c stage=prod -c region=ap-southeast-1 -c sesIdentityEmail=admin@example.com
```
